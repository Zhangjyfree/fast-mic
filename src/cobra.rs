...
fn solve_fba_max_biomass(
    ...
    Ok(FBAResult { objective_value: obj_value, flux_distribution: fd })
}
...
fn cycle_free_pfba(
    ...
    lp = lp.with(bio_cap.leq(fba_biomass));
    let bio_floor: Expression = 1.0 * flux_vars[bio_idx];
    // Lenient floor: allow CFF to sacrifice the objective down to 0 if cycle
    // removal demands it. Adequate for single-species and pairwise co-culture
    // pFBA (objective has non-zero |v| cost so this rarely matters).
    lp = lp.with(bio_floor.geq(0.0));
    ...
    match lp.solve() {
        Ok(sol) => {
            ...
            Ok(Some(FBAResult { objective_value: new_growth, flux_distribution: fd }))
        }
        Err(_) => Ok(None),
    }
}
